...
public class IntegerOutputRecipeSerializer implements RecipeSerializer<RecipeHolder<IRecipeInput, Integer>>
{
	...
	public RecipeHolder<IRecipeInput, Integer> fromNetwork(ResourceLocation id, RegistryFriendlyByteBuf buf)
	{
	...
	public void toNetwork(RegistryFriendlyByteBuf buf, RecipeHolder<IRecipeInput, Integer> recipe)
	{
		buf.writeByte(0);
	...
	public MapCodec<RecipeHolder<IRecipeInput, Integer>> codec()
	{
		return new MapCodec<>()
		{
			...
			@Override
			public <T> DataResult<RecipeHolder<IRecipeInput, Integer>> decode(DynamicOps<T> ops, MapLike<T> input)
			{
				...
				{
					return DataResult.error(() -> "Failed to decode IC2 integer output recipe: " + e.getMessage());
				}
			}

			@Override
			public <T> RecordBuilder<T> encode(RecipeHolder<IRecipeInput, Integer> input, DynamicOps<T> ops, RecordBuilder<T> prefix)
			{
				return prefix.withErrorsFrom(DataResult.error(() -> "Encoding IC2 integer output recipes not supported"));
			}
		};
	}
